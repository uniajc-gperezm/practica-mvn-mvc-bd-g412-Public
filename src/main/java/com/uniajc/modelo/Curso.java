...
public class Curso {
  ...
  public Curso(String nombre, String materia) {
    this.nombre = nombre;
    this.materia = materia;
  }

  // Constructor para crear estudiante con id (para consultas y actualizaciones)
  public Curso(int id, String nombre, String materia) {
    this.id = id;
    this.nombre = nombre;
    this.materia = materia;
  }
  ...
  public String toString() {
    if (this.nombre != null) {
      return this.nombre + " (id=" + this.id + ") - " + (this.materia != null ? this.materia : "");
    }
  ...
  public static void insertarEstudiante(Estudiante estudiante) {
    String sql = "INSERT INTO estudiantes (nombre, materia_institucional) VALUES (?, ?)";
    try {
      try (Connection conexion = ConexionDatabase.getConnection();
          PreparedStatement preparedStatement = conexion.prepareStatement(sql)) {
        preparedStatement.setString(1, estudiante.getNombre());
        preparedStatement.setString(2, estudiante.getmateria());
